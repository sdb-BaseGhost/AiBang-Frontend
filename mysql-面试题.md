...
**第三范式（3NF）：在2NF基础上，任何非主属性

 

 (opens new window)不依赖于其它非主属性（在2NF基础上消除传递依赖）**
...
### 第14题：sql题：给学生表、课程成绩表，求不存在01课程但存在02课程的学生的成绩
...
### 第15题：给定一个学生表 student_score（stu_id，subject_id，score），查询总分排名在5-10名的学生id及对应的总分
...
### 第21题：说一下mysql的innodb与MyISAM的区别？
...
- 如果用到了索引，那么查询的时候，可以基于二分查找算法，通过索引快速定位到目标数据，MySQL 索引的数据结构一般是 B+ 树，其搜索复杂度为 O(log_dN)，其中 d 表示节点允许的最大子节点个数。
...
### 第31题：表中十个字段，你主键用自增ID还是UUID，为什么？
...
### 第32题：什么自增ID更快一些，UUID不快吗，它在B+树里面存储是有序的吗?
...
### 第34题：查询数据时，到了B+树的叶子节点，之后的查找数据是如何做？
...
### 第43题：联合索引ABC，现在有个执行语句是A = XXX and C &lt; XXX，索引怎么走

根据最左匹配原则，A可以走联合索引，C不会走联合索引，但是C可以走索引下推

---

### 第44题：联合索引(a,b,c) ，查询条件 where b &gt; xxx and a = x 会生效吗

索引会生效，a 和 b 字段都能利用联合索引，符合联合索引最左匹配原则。

---

### 第45题：联合索引 (a, b，c)，where条件是 a=2 and c = 1，能用到联合索引吗？
...
### 第49题：如果一个列即使单列索引，又是联合索引，单独查它的话先走哪个？
...
### 第50题：索引已经建好了，那我再插入一条数据，索引会有哪些变化？
...
### 第52题：如果有一个字段是status值为0或者1，适合建索引吗
...
### 第63题：可重复读隔离级别下，A事务提交的数据，在B事务能看见吗？
...
- 回滚记录会占用大量存储空间，事务回滚时间长。在MySQL

 

 (opens new window)中，实际上每条记录在更新的时候都会同时记录一条回滚操作。记录上的最新值，通过回滚操作，都可以得到前一个状态的值，sql 越多，所需要保存的回滚数据就越多。 
...
### 第72题：MySQL两个线程的update语句同时处理一条数据，会不会有阻塞？
...
### 第73题：两条update语句处理一张表的不同的主键范围的记录，一个&lt;10，一个&gt;15，会不会遇到阻塞？底层是为什么的？
...
### 第85题：为什么要写RedoLog，而不是直接写到B+树里面？
...
### 第86题：mysql 两次写（double write buffer）了解吗？
...
### 第91题：如果Explain用到的索引不正确的话，有什么办法干预吗？
